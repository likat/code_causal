## FUNCTION FOR WFPBB-PENCOMP
## for the H-L diagnostic (stratified by treatment group)
## observed Y, predicted Y, number of observed Y_i, quantiles of predicted p
## compare to chisq on 8 degrees of freedom
...
calculate.pencomp.wfpbb <- function(propen_model, # string for propensity model, e.g. "A~X+Z"
      ...
      modtrtcoef <- matrix(nrow = F_draw, ncol = 15)
      sigmaspl <- list(rep(NA, F_draw), rep(NA, F_draw))
      hl_modtrt <- rep(NA, F_draw)
      hl_outcome <-list(rep(NA, F_draw), rep(NA, F_draw))
      ...
      for(f in 1:F_draw){
        ...
        tryCatch ( {        
          ...
          # hl_modtrt[f] <- hltest(mod_trt)$statistic %>% as.numeric
          modtrtcoef[f,] <- mod_trt$coefficients
          popbootpolya$phat <-
          ...
          numflag=0
            ...
            sigmaspl[[trtgrp+1]][f] <- psppM$summary("sigma_spline")$estimate
            if(trtgrp==0){
            ...
            ## H-L statistic
            # hl_outcome[[trtgrp+1]][f] <- get.hl(obs = popbootpolya$fsbenefit_12mo,
            #                                     pred= impute_potenoutpop,
            #                                     prob = imputeF(
            #                                       newdata= popbootpolya,
            #                                       # model = psppM,
            #                                       fixedcoef=psppM$summary("beta")$estimate,
