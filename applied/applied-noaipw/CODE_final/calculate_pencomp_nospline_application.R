...
calculate.linimpute <- function(outc_model,
  ...
  modcoeff <- vector(mode = "list", length = 2)
  coefse <- vector(mode = "list", length = 2)
  potenout <- c(NA,NA)
  ...
  for(trtgrp in 0:1){
    ...
    modcoeff[[trtgrp+1]] <- mod_impute$coefficients
    ...
    coefse[[trtgrp + 1]] <- summary(mod_impute)$coefficients[,2]
    if(trtgrp == 0){
