...
if(type_outcome == "gb"){
  pop <- pop %>% mutate(
    mu1 = cluseff + bA + bx1*X1 + bAx1*X1 + bz*Z+ bAz*Z,
    mu0 = cluseff + bx1*X1 + bz*Z
  )
  print(type_outcome)
  ...
  #  )
  pop <- pop %>% mutate(
    mu1 = cluseff +bA + bx1*X1 + bAx1*X1  + bz*Z,
    mu0 = cluseff +bx1*X1 + bz*Z
  )
  print(type_outcome)
...
varnames_outcome <-  c("X1", "Z")
varnames_outcome_generalize <- c(x_name_mod, z_name, paste0(x_name_mod, ":", z_name))
# outcome_mod <- as.formula("Ysamp ~ X2 + Z")
outcome_mod_nospline <- as.formula("Ysamp ~ X1  + Z")
propensity_mod <- as.formula("A~ X2")
outcome_mod <- as.formula("Ysamp ~ X1 + Z")
...
knotloc_pencomp <- vector(mode = "list", length = 2)
knotloc_pencompwfpbb <- vector(mode="list", length = 2)
...
knotloc_pencomp_boot <- vector(mode = "list", length = 2)
knotloc_pencompwfpbb_boot <- vector(mode = "list", length = 2)
...
while(boot <= numboot){ ## START bootstrap loop
  ...
  # wtdres_wfpbb <- calculate.wtd(imputedlist = imputedlist_wfpbbspline, bootdf=samp)
  wtdest_pencomp_boot[boot,] <- c(wtdres[[1]],
                                  wtdres[[2]])
  # wtdest_pencompwfpbb_boot[boot,] <- c(imputedlist_wfpbbspline$wtdest)
  ...
  knotloc_pencomp_boot <- Map(rbind, knotloc_pencomp_boot, imputedlist_spline$knotlocations)
  knotloc_pencompwfpbb_boot <- Map(rbind, knotloc_pencompwfpbb_boot,imputedlist_wfpbbspline$knotlocations)
...
# knotloc_pencomp <- Map(rbind, knotloc_pencomp, lapply())
print(paste0("I is ",i))
...
res <- list(patetrue = pate_true, sate = sate,
            ...
            twostage_pencomp = NULL,
            twostage_pencompwfpbb = NULL,
            twostage_nospline = NULL,
...
saveRDS(res, file = paste0("results-raw-2stage-wrongptrt-rightout/", type_outcome, type_overlap,"/results_tingting_noaiptw", type_outcome, type_overlap,i, ".RDS"))
