...
## PARAMETERS FOR aipw
  ...
  propensity_mod_mis <- as.formula("A~X1 + X2")
...
i<- as.numeric(i)
  ...
  # pop$I <- rbinom(nrow(pop), size = 1, prob = 10000/nrow(pop))
  samp <- pop[pop$I==1,]
  samp$wts <- npop*normalize(1/samp$pselect1stage)
  # samp$wts <- 1/(10000/nrow(pop))
...
aipw_bt_allwrong <- rep(NA,numboot)
  ...
  # aipw_unwtd_correcttrt <-aipw_wtd_wtdtrt <-aipw_wtd_mistrt <-aipw_unwtd_wtdtrt <- vector(mode = "list", length = 4)
  # aipw_unwtd_correcttrt_noz <-aipw_wtd_wtdtrt_noz <-aipw_wtd_mistrt_noz <-aipw_unwtd_wtdtrt_noz <- vector(mode = "list", length = 4)
  # aipw_correct_bench <-aipw_ptrtwrong_bench <-aipw_outwrong_bench  <- vector(mode = "list", length = 4)
  # 
  # aipw_est_unwtd_correcttrt = aipw_temp$pate_unwtd_correcttrt ## both treatment and outcome models weighted
  # aipw_est_wtd_wtdtrt = aipw_temp$pate_wtd_wtdtrt ## only outcome model weighted
...
saveRDS(res, file = paste0("results-raw-2stage/aipw/results_tingting_aipw", type_outcome, type_overlap,i,".RDS"))
