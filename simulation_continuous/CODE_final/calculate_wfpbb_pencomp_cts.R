...
calculate.wfpbb.pencomp <- function(propen_model, # string for propensity model, e.g. "A~X+Z"
    ...
    tryCatch ( {        
      ...
      for(trtgrp in 0:1){
        ...
        psppM=lme(response ~ covariateX-1, random=list(all=pdIdent(~0+linearB)),
                  control=(maxIter=100))
        # print(xtable(summary(psppM)$tTable, digits=3, type = "latex"), file = "coeff_pencomp_bootY1.tex")
        ...
        if(trtgrp==0){
          ...
          # coefflist0[[f]] <- list(
          #   fixed = psppM$summary("beta")$estimate, 
          #   random = psppM$summary("gamma")$estimate,
          #   sigma_y = psppM$summary("sigma_y")$estimate
          # )
        }else{
        ...
        if(trtgrp == 0){
          impute_potenout <-
            imputeF(newdata=samp[samp[,trt.varname]!=trtgrp,],
                    ...
                    propen.score.new=logit(1-expit(phat_samp[samp[,trt.varname]!=trtgrp])))
        ...
        if(trtgrp == 0){
          ...
          impute_potenoutpop <- 
            imputeF(
              ...
              knotloc = knots,
              outcome.model = popmod,
              propen.score.new=logit(1-expit(phat_pop))
            )
          ...
          knotlocations[[1]] <- rbind(knotlocations[[1]], knots)
        }
        if(trtgrp == 1){
          ...
          knotlocations[[2]] <- rbind(knotlocations[[2]], knots)
        }
      }
  ...
  res <- list(imputed0 = imputed_ctrl, 
              imputed1= imputed_trt,
              mod_trt = mod_trt, 
              ...
              knotlocations = lapply(knotlocations, colMeans),
              # phatsamp= phat_samp,
              # phatpop = phat_pop,
