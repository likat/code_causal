...
getres <- function(type_outcome, type_overlap, type_outmod){
  ...
  time <- data.frame(pselspl = 0, correct = 0, noz = 0)
  itersum <- 0
  skipnum = 0
  for(i in 1:iter){
    temp <-
    res_pselspl <- readRDS(paste0("../results-raw-2stage/wrongptrt/results_wrongptrt_",type_outmod, "_",type_outcome, type_overlap,"_pselspl",i,".RDS"))
    res_allother <- readRDS(paste0("../results-raw-2stage/wrongptrt/results_wrongptrt_",type_outmod, "_",type_outcome, type_overlap,"_noselspl",i,".RDS"))
    
    time$pselspl = time$pselspl+res_pselspl$time/iter
    time$correct = time$correct+res_allother$time/iter
    time$noz = time$noz +res_pselspl$time/iter
	  ...
	  synthpop_noz <- Map(c, synthpop_noz, res_allother$synthpop_noz)
	  synthpop_correct <- Map(c, synthpop_correct, res_allother$synthpop_correct)
	  synthpop_pselspl <- Map(c, synthpop_pselspl, res_pselspl$synthpop_pselspl)
	  itersum <- itersum + 1
  ...
  temp <- readRDS(paste0("../results-raw-2stage/wrongptrt/results_wrongptrt_",type_outmod, "_",type_outcome, type_overlap,"_pselspl",1,".RDS"))
  ...
  reswrite <- list(
    ...
    # summary_wtdest_noz = wtdest_noz %>% get.summary(truex = temp$patetrue, wfpbbstatus = T, L = 200),
    # summary_wtdest_correct = wtdest_correct%>% get.summary(truex = temp$patetrue, wfpbbstatus = T, L = 200),
    # summary_wtdest_pselspl = wtdest_pselspl%>% get.summary(truex = temp$patetrue, wfpbbstatus = T, L = 200),
    summary_synthpop_noz = synthpop_noz%>% get.summary(truex = temp$patetrue, wfpbbstatus = T, L = 200),
    summary_synthpop_correct = synthpop_correct%>% get.summary(truex = temp$patetrue, wfpbbstatus = T, L = 200),
    summary_synthpop_pselspl = synthpop_pselspl%>% get.summary(truex = temp$patetrue, wfpbbstatus = T, L = 200)
    # summary_twostage_noz = twostage_noz%>% get.summary(truex = temp$patetrue, wfpbbstatus = T, L = 200),
    # summary_twostage_correct= twostage_correct%>% get.summary(truex = temp$patetrue, wfpbbstatus = T, L = 200),
    # summary_twostage_pselspl= twostage_pselspl%>% get.summary(truex = temp$patetrue, wfpbbstatus = T, L = 200)
...
for(outcome in 1:length(outcometypes)){
  for(overlap in 1:length(overlaptypes)){
    ...
    # reslist[[outmod]] <- getres(type_outcome = outcomecurrent, type_overlap = overlapcurrent,type_outmod =outmodcurrent)#, file = paste0("results-clean/res_wrongptrt_",outmodcurrent,outcomecurrent,overlapcurrent,".RDS"))
    
saveRDS(getres(type_outcome = outcomecurrent, type_overlap = overlapcurrent,type_outmod =outmodcurrent), file = paste0("res_wrongptrt_",outmodcurrent,outcomecurrent,overlapcurrent,".RDS"))
    }
  }
}
