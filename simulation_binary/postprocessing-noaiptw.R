...
get.summary <- function(x, truex, typewfpbb = F){
  ...
  if(typewfpbb == T){
    ...
    rmse <- sqrt(mean((x$est - truex)^2))
    x$mivar <- x$mivar*fpc
    tdf <- round((20-1)*(1+x$withinvar/((20+1)*x$btwnvar)^2))
...
getres <- function(type_outcome, type_overlap, iter){
  ...
  temp <- readRDS(paste0("../results-raw/results_tingting_noaiptw", type_outcome, type_overlap,"2.RDS"))
...
refdf <- data.frame(
  ...
  numiter = c(250, 250, 330, 375)
)
for(j in 1:nrow(refdf)){
  outcomecurrent <- refdf$outcometypes[j]
  overlapcurrent <- refdf$overlaptypes[j]
  itercurrent <- refdf$numiter[j]
  restemp <- getres(type_outcome = outcomecurrent, type_overlap = overlapcurrent, iter = itercurrent)
  print(restemp$gooditer)
