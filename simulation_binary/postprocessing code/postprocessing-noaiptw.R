...
getres <- function(type_outcome, type_overlap, iter){
  ...
  for(i in 1:iter){
    ...
    temp <- tryCatch ( {
      res <- readRDS(paste0("../results-raw-2stage-bin/", type_outcome, type_overlap, "/results_tingting_noaiptw", type_outcome, type_overlap,i,".RDS"))
       }, error=function(e) return(NA) )
    if(sum(is.na(temp))>0){skipnum = skipnum + 1; next}
    res <- readRDS(paste0("../results-raw-2stage-bin/", type_outcome, type_overlap, "/results_tingting_noaiptw", type_outcome, type_overlap,i,".RDS"))
    res2 <- readRDS(paste0("../results-raw-2stage-bin/linimpute-wrong/results_tingting_noaiptw", type_outcome, type_overlap,i, ".RDS"))
  ...
  temp <- readRDS(paste0("../results-raw-2stage-bin/", type_outcome, type_overlap, "/results_tingting_noaiptw", type_outcome, type_overlap,2,".RDS"))
...
# for(j in 1:1){
  ...
  saveRDS(restemp, file = paste0("../results-clean/res_",outcomecurrent,overlapcurrent,".RDS"))
  print(j)
}
# outcomecurrent = 'gb'
