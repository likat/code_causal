res <- readRDS("../results-raw-2stage-bin/aipw/results_tingting_aipwgbnotshared1.RDS")
numiter = 200
type_outcome = "gb"
...
get.overall.aipw <- function(numiter=200, type_outcome, type_overlap){
  tempres <- 
    readRDS(paste0("../results-raw-2stage-bin/aipw/results_tingting_aipw", type_outcome, type_overlap, "1.RDS"))
ind_estonly <- (2:(length(tempres)-1))
...
for(i in 1:numiter){
  ...
  tempres <- readRDS(paste0(
    "../results-raw-2stage-bin/aipw/results_tingting_aipw", type_outcome, type_overlap, i, ".RDS"
  ))
...
for(j in 1:nrow(refdf)){
  ...
  saveRDS(restemp$raw, file = paste0("../results-clean/res_",outcomecurrent,overlapcurrent,"_aipw_raw_bin.RDS"))
  saveRDS(restemp$overall, file = paste0("../results-clean/res_",outcomecurrent,overlapcurrent,"_aipw_bin.RDS"))
