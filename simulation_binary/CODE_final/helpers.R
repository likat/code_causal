...
combine.aipw <- function(est, boottbl, popsize,sampsize, num_boot = numboot){
  fpc <- (popsize - sampsize) / (popsize-1)
  tdf <- num_boot - 1
  # est <-  mean(boottbl, na.rm=T)
  est <-  est
  varest_nofpc <- var(boottbl, na.rm=T)
  varest <- fpc*varest_nofpc
  cilower <- est - qt(0.975, tdf)*sqrt(varest)
  ciupper <- est + qt(0.975, tdf)*sqrt(varest)
  cilower_nofpc <- est - qt(0.975, tdf)*sqrt(varest_nofpc)
  ciupper_nofpc <- est + qt(0.975, tdf)*sqrt(varest_nofpc)
  ...
  res <- data.frame(
    ...
    cilower = cilower,
    ciupper = ciupper,
    cilower_nofpc = cilower_nofpc,
    ciupper_nofpc = ciupper_nofpc
  )
}

## COMBINING FUNCTION FOR AIPTW coefficients
combine.aiptw.coef <- function(est, boottbl, num_boot = numboot){
  tdf <- num_boot - 1
  varest <- apply(boottbl, 2, var)# var(boottbl, na.rm=T)
  cilower <- est - qt(0.975, tdf)*sqrt(varest)
  ciupper <- est + qt(0.975, tdf)*sqrt(varest)
...
combine.rubin.applied <- function(boottbl, num_boot=numboot){
...
combine.rubin.2stage <- function(boottbl, num_boot=numboot, numstrat, numclus){
  est <- mean(boottbl[,1], na.rm=T)
  withinvar <- mean(boottbl[,2], na.rm=T)
  btwnvar <- var(boottbl[,1], na.rm=T)
  mivar <- withinvar + (1+1/num_boot)*btwnvar
  tdf <- min(numclus - numstrat, round((num_boot-1)*(1+withinvar/((num_boot+1)*btwnvar)^2)))
  cilower_t <- est - qt(0.975,tdf)*sqrt(mivar)
  ciupper_t <- est + qt(0.975,tdf)*sqrt(mivar)
  cilower_emp <- quantile(boottbl[,1],c(0.025, 0.975), na.rm=T)[1]
  ciupper_emp <- quantile(boottbl[,1],c(0.025, 0.975), na.rm=T)[2]
...
combine.rubin.cate <- function(boottbl, num_boot=numboot){
  est <- colMeans(boottbl$estmat)
  withinvar <- colMeans(boottbl$varmat)
  btwnvar <- apply(boottbl$estmat, 2, var)
  mivar <- withinvar + (1+1/num_boot)*btwnvar
  tdf <- round((num_boot-1)*(1+withinvar)/((num_boot+1)*btwnvar)^2)
  cilower_t <- est - qt(0.975,tdf)*sqrt(mivar)
  ciupper_t <- est + qt(0.975,tdf)*sqrt(mivar)
  cilower_emp <- apply(boottbl$estmat,2,quantile, c(0.025, 0.975))[1,]
  ciupper_emp <- apply(boottbl$estmat,2,quantile, c(0.025, 0.975))[2,]
  cicov_t <- between(cate_true, cilower_t, ciupper_t)
  cicov_emp <- between(cate_true, cilower_emp, ciupper_emp)
  res <- list(est=est, withinvar=withinvar,btwnvar=btwnvar,mivar=mivar,
              ci_tlength = ciupper_t - cilower_t,
              ci_emplength = as.numeric(ciupper_emp- cilower_emp),
              cicov_t = cicov_t, cicov_emp = cicov_emp)
